...
class Subscription {
  ...
  constructor(api, did, ccode, detailed = false) {
    this._api = api;
    this._did = did;
  ...
  async subscribe(callback) {
    ...
    const tokenData = await getTokenData(this._ccode, this._api);
    if(this._detailed) {
      await this._api.query.tokens.accounts(sanitiseDid(this._did), sanitiseCCode(this._ccode), (data) => {
        if (this._subscribed) {
      ...
      await this._api.query.tokens.accounts(sanitiseDid(this._did), sanitiseCCode(this._ccode), (data) => {
        if (this._subscribed) {
          console.log(this._did, (data.toJSON())['data'].free /(Math.pow(10, tokenData.decimal)));
 ...
 * @param {String} did
 * @param {String} currencyCode
 * @param {Function} callback Cb function to execute with new token balance in Highest Form
 * @param {ApiPromise} api
...
module.exports = {
  ...
  mintToken,
  getTokenBalance,
  subscribeToGetTokenBalance,
  getDetailedTokenBalance,
  subscribeToGetDetailedTokenBalance,
  getLocks,
  getTokenIssuer,
  getTokenList,
  getTokenData,
